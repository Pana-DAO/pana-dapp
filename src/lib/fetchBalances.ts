...
const fetchBalances = (addressOrENS: string, NetworkId: NetworkId, quoteCurrency = "usd") =>
  fetch(
    `${CovalentApi}/${NetworkId}/address/${addressOrENS}/balances_/?quote-currency=${quoteCurrency}&key=${COVALENT_KEY}`,
  ).then(d => d.json());
...
export const fetchCrossChainBalances = async (address: string) => {
  ...
  return {
    karsha: addressBalancesByNetwork(Networks, balances, "GPANA_ADDRESS"),
  };
};
