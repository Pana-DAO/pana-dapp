...
import TokenStack, { PanaTokenProps, PanaTokenStackProps, Token } from "src/lib/PanaTokenStack";
...
const Borrow = ({
  ...
  Icon1: PanaTokenProps["name"];
  borrowableTokensIcons: PanaTokenStackProps["tokens"];
  borrowOn: string;
  href: string;
}) => {
...
const WalletTotalValue = () => {
  ...
  const walletValue = {
    ...
    PANA: walletTotalValueUSD / marketPrice,
  };
  return (
    <Box>
      ...
      <Box sx={{ padding: theme.spacing(0, 3), display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", padding: theme.spacing(3, 0) }}>
          ...
          <CloseButton size="small" onClick={onClose} aria-label="close wallet">
            <SvgIcon viewBox="0 0 25 25" component={Close} />
          </CloseButton>
        </Box>
          ...
          <Grid container spacing={2}>
            <>
              <Grid item xs={12} sm={4}>
                <Typography>pPana Balance:</Typography>
              </Grid>
        ...
        >
          <Box sx={{ display: "flex", flexDirection: "column" }} style={{ gap: theme.spacing(1.5) }}></Box>
        </Box>
