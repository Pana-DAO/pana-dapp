...
import {
  ...
  onStakeAssets,
  onUnstakeAssets,
} from "src/slices/StakingPoolsSlice";
  ...
  const onStakeUnstake = async () => {
    if (stake == "stake") {
      if (quantity === "" || Number(quantity) <= 0) {
        ...
        dispatch(error(`Max staking is ${assetBalance} ${farm.symbol}. Click Max to autocomplete.`));
      } else {
        dispatch(
          onStakeAssets({
      ...
      if (quantityUnstake === "" || Number(quantityUnstake) <= 0) {
        ...
        dispatch(
          error(
            `Max unstaking is ${ethers.utils.formatUnits(userPoolBalance[farm.pid], farm.decimals)} ${
              farm.symbol
            ...
            <Box id="modal-modal-description" display="flex" flexDirection="row" className="farm-price-data-row">
              <div className="farm-price-data">
                ...
                <Typography variant="h3" color="primary" className="price">
                  {userPoolBalance && userPoolBalance[farm.pid]
                    ? formatCurrency(+ethers.utils.formatUnits(userPoolBalance[farm.pid], farm.decimals), 6, "PANA") +
                      " " +
                      farm.symbol
                    : "-"}
                </Typography>
              ...
              <div className="farm-price-data">
                ...
                <Typography variant="h3" color="primary" className="price">
                  {pendingPanaForUser && pendingPanaForUser[farm.pid]
                    ? formatCurrency(+ethers.utils.formatUnits(pendingPanaForUser[farm.pid], 18), 4, "PANA") + " Pana"
                    : "-"}
                </Typography>
              </div>
            </Box>
                          ...
                          {stake == "stake" ? (
                            ...
                            <FormControl className="pana-input" variant="outlined">
                              ...
                              <OutlinedInput
                                ...
                                endAdornment={
                                  <InputAdornment onClick={setMaxUnstake} position="end">{t`Max`}</InputAdornment>
                                }
                                aria-describedby="outlined-weight-helper-text"
                                inputProps={{
                                  "aria-label": "Amount",
                    ...
                    <Box display="flex" className="flxrow data-row" flexDirection="row" justifyContent="space-between">
                      ...
                      <Typography className="price-data">
                        {isFarmLoading ? (
                          ...
                          `${formatCurrency(+ethers.utils.formatUnits(panaPerDay, 18), 4, "PANA")} Pana`
                        )}
                      </Typography>
                    </Box>
                  )}
