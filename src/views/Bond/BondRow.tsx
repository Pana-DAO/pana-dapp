...
import { DisplayBondDiscount, DisplayBondPrice } from "./Bond";
import TokenStack from "src/lib/PanaTokenStack";

export function BondDataCard({ bond, networkId }: { bond: IBond; networkId: NetworkId }) {
  const isBondLoading = useAppSelector(state => state.bonding.loading);

  return (
    <Slide direction="up" in={true}>
      <Paper id={`${bond.index}--bond`} className="bond-data-card pana-card">
        ...
        <div className="data-row">
          ...
          <Typography>
            {isBondLoading ? <Skeleton width="50px" /> : <DisplayBondDiscount showNumber={false} key={bond.index} bond={bond} />}
          </Typography>
        </div>
        <div className="data-row">
          <Typography>
...
export function BondTableData({ bond, networkId }: { bond: IBond; networkId: NetworkId }) {
